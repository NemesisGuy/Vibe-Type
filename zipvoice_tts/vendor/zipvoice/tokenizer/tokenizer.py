...
try:
    ...
    logging.getLogger(__name__).warning(
        "piper_phonemize not installed; espeak tokenizer will be unavailable"
    )
    _PHONEMIZE_ERROR = ex

    def phonemize_espeak(*args, **kwargs):  # type: ignore[override]
        raise RuntimeError(
            "The espeak tokenizer requires `piper_phonemize`. Install it via `pip install"
            " piper_phonemize -f https://k2-fsa.github.io/icefall/piper_phonemize.html`."
        ) from _PHONEMIZE_ERROR
